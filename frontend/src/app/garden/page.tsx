  ...
  return (
    <div>
      <header className="page-head">
        <h1 className="page-title">数字花园</h1>
      ...
      {nodes.length === 0 ? (
        ...
        <>
          ...
          <section className="mt-12 max-w-content">
            <h2 className="section-title">引用最多的笔记</h2>
              ...
              {nodes
                ...
                .map((node) => (
                  <li key={node.id}>
                    <a
                      href={`/posts/${node.id}`}
                      className="card card-hover flex items-center justify-between p-3.5"
                    >
                      <span className="truncate text-sm font-medium">{node.title}</span>
                      <span className="badge badge-accent ml-3 shrink-0">
                        {node.degree}
                      </span>
                    </a>
                  </li>
